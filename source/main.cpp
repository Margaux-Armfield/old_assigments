...
bool shadowFeeler(vec4 p0, Object *object){
  bool inShadow = false;
//    castRay(p0, lightPosition, object, 0);
    vec4 L = lightPosition - p0;
    double len = length(L);
    
    for(unsigned int i=0; i < sceneObjects.size(); i++){
        L = normalize(L);
        Object::IntersectionValues intersect = sceneObjects[i]->intersect(p0, L);
        if (intersect.t_w < len) {
            inShadow = true;
        }
    }
...
vec4 castRay(vec4 p0, vec4 E, Object *lastHitObject, int depth){
  ...
  //Raytracing code here
    ...
    if(min_intersect.t_w != std::numeric_limits<double>::infinity()){
        Object* int_objects = sceneObjects[min_intersect.ID_];
        if (shadowFeeler( min_intersect.P_w, int_objects) == false) {
       vec4 L = normalize(lightPosition - min_intersect.P_w);

        double D = dot(L, min_intersect.N_w);

        vec4 R = normalize(reflect(L, min_intersect.N_w));

        double S = fmax(0, pow(dot(E, R), int_objects->shadingValues.Kn));
        
        vec4 A = fmax(0, int_objects->shadingValues.Ka);
            
        color = int_objects->shadingValues.color;
            
            color4 material_ambient(int_objects->shadingValues.color.x* int_objects->shadingValues.Ka,
                                    int_objects->shadingValues.color.y* int_objects->shadingValues.Ka,
                                    int_objects->shadingValues.color.z* int_objects->shadingValues.Ka, 1.0 );
            
            color4 material_diffuse(int_objects->shadingValues.color.x,
                                    int_objects->shadingValues.color.y,
                                    int_objects->shadingValues.color.z, 1.0 );
           
            color4 material_specular(int_objects->shadingValues.Ks,
                                     int_objects->shadingValues.Ks,
                                     int_objects->shadingValues.Ks, 1.0 );
            
            color4 ambient_product  = A * material_ambient;
            color4 diffuse_product  = D * material_diffuse;
            color4 specular_product = S * material_specular;
